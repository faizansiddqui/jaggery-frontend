    ...
    return (
        <div className="border border-red-700 p-4 md:p-5 grid grid-cols-1 lg:grid-cols-12 gap-4 items-start">
            <div className="lg:col-span-8 flex flex-col gap-2">
                ...
                <p className="font-brand text-2xl leading-none">{contact.name}</p>
                <p className="font-headline text-[10px] tracking-widest opacity-60">{contact.email}</p>
                <p className="font-headline text-xs leading-6 opacity-90 whitespace-pre-wrap">{contact.message}</p>
                {contact.resolutionMessage ? (
                    <p className="font-headline text-[10px] tracking-widest text-green-300/90">Resolution: {contact.resolutionMessage}</p>
                ) : null}
            </div>
