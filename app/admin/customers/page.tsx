        ...
        <div className="flex flex-col gap-12">
            ...
            <div className="flex flex-col md:flex-row gap-3 md:items-center md:justify-between">
                <div className="flex gap-2 overflow-x-auto">
                    {(['ALL', 'ACTIVE', 'NEW', 'BLOCKED'] as SegmentFilter[]).map((entry) => (
                        <button
                            key={entry}
                            ...
                            className={`px-4 py-2 font-headline text-[10px] uppercase tracking-widest border transition-colors whitespace-nowrap ${segment === entry
                                    ? 'border-[#b90c1b] text-[#b90c1b]'
                                    : 'border-[#ffffff]/10 opacity-50 hover:opacity-100'
                                }`}
                        >
                            {entry}
                        </button>
                ...
                <div className="flex gap-2 w-full md:w-auto">
                    ...
                    />
                    <button onClick={loadCustomers} className="px-5 bg-[#1c1b1b] border border-[#ffffff]/10 font-headline text-[10px] uppercase tracking-widest hover:border-[#b90c1b]">Refresh</button>
                </div>
            </div>
            ...
            <div className="bg-[#1c1b1b] border border-[#ffffff]/10 overflow-hidden">
                ...
                <div className="flex flex-col">
                    ...
                    {filteredCustomers.map((c, i) => {
                        const isBlocked = String(c.status || '').toUpperCase() === 'BLOCKED';
                        const busy = busyEmail === c.email;

                        return (
                            <div key={`${c.email}-${i}`} className="grid grid-cols-12 gap-8 p-8 border-b border-[#ffffff]/5 last:border-none hover:bg-[#ffffff]/2 transition-colors items-center">
                                <div className="col-span-1 font-headline text-[10px] uppercase opacity-40">{String(i + 1).padStart(2, '0')}</div>
                                ...
                                <div className="col-span-2 font-brand text-3xl">{c.ordersCount}</div>
                                <div className="col-span-2 font-brand text-3xl text-[#b90c1b]">{formatMoney(c.totalSpent, currency)}</div>
                                <div className="col-span-1 flex justify-end">
                                    <span className={`px-4 py-1 font-headline text-[8px] uppercase tracking-widest font-black ${statusTone(c.status)}`}>{c.status}</span>
                                </div>
                                <div className="col-span-2 flex justify-end gap-2">
                                    <button
                                        onClick={() => openActivity(c.email)}
                                        className="px-3 py-2 font-headline text-[9px] uppercase tracking-widest border border-[#ffffff]/15 hover:border-[#b90c1b]"
                                    >
                                        Activity
                                    </button>
                                    <button
                                        disabled={busy}
                                        onClick={() => toggleCustomerStatus(c.email, isBlocked)}
                                        className={`px-3 py-2 font-headline text-[9px] uppercase tracking-widest border disabled:opacity-40 ${isBlocked
            ...
            {(loadingActivity || activity) && (
                <div className="fixed inset-0 z-[120] bg-black/70 p-4 md:p-10 overflow-y-auto">
                    <div className="max-w-5xl mx-auto bg-[#1c1b1b] border border-[#ffffff]/15 p-6 md:p-8">
                        <div className="flex items-start justify-between gap-4 border-b border-[#ffffff]/10 pb-4 mb-6">
                            <div>
                        ...
                        {activity && !loadingActivity && (
                            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                                <div className="lg:col-span-4 space-y-4">
                                    <div className="border border-[#ffffff]/10 p-4">
                                ...
                                <div className="lg:col-span-8">
                                    <h4 className="font-brand text-3xl uppercase tracking-widest border-b border-[#ffffff]/10 pb-3 mb-4">Timeline</h4>
                                    <div className="max-h-[60vh] overflow-y-auto pr-1 space-y-3">
                                        ...
                                        {activity.timeline.map((entry, index) => (
                                            <div key={`${entry.type}-${entry.occurredAt || index}`} className="border border-[#ffffff]/10 p-4">
