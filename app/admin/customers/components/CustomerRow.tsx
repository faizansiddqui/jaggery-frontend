...
export default function CustomerRow({
    ...
    return (
        <div className="grid grid-cols-12 gap-8 p-8 border-b border-[#ffffff]/5 last:border-none hover:bg-[#ffffff]/2 transition-colors items-center">
            <div className="col-span-1 font-headline text-[10px] uppercase opacity-40">{String(index + 1).padStart(2, '0')}</div>
            <div className="col-span-4 flex items-center gap-6">
                ...
                <div className="flex flex-col gap-1">
                    <span className="font-brand text-2xl uppercase tracking-widest">{customer.name}</span>
                    <span className="font-headline text-[9px] uppercase tracking-widest opacity-40">{customer.email}</span>
                </div>
            </div>
            ...
            <div className="col-span-1 flex justify-end">
                <span className={`px-4 py-1 font-headline text-[8px] uppercase tracking-widest font-black ${statusClass}`}>{normalizedStatus}</span>
            </div>
            <div className="col-span-2 flex justify-end gap-2">
                <button
                    onClick={() => onOpenActivity(customer.email)}
                    className="px-3 py-2 font-headline text-[9px] uppercase tracking-widest border border-[#ffffff]/15 border-secondary"
                >
                    Activity
                </button>
                <button
