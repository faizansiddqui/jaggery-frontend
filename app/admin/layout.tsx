...
export default function AdminLayout({
  ...
  if (!ready) return <div className="min-h-screen bg-background text-on-background" />;
  ...
  const menuItems = [
    ...
    { name: 'Banners', icon: 'slideshow', href: '/admin/banners' },
    { name: 'Testimonials', icon: 'format_quote', href: '/admin/testimonials' },
  ...
  return (
    <div className="admin-ui flex min-h-screen bg-background text-on-background">
      ...
      {/* Main Content */}
      <main id="admin-scroll-root" className="flex-1 ml-64 h-screen p-8 md:p-12 overflow-y-auto bg-background text-on-background">
        {children}

        {/* Footer info for admin */}
        <div className="mt-20 pt-8 border-t border-outline/10 flex justify-between items-center opacity-30 font-headline text-[9px] tracking-widest">
          <span> {settings.navbarTitle || settings.siteName || 'Amila Gold'} © 2026</span>
        </div>
      </main>
    </div>
  );
